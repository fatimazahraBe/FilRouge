# Analyse des Prix Alimentaires Mondiaux (2015–2025)

## Description du Projet

Ce projet vise à analyser les déterminants économiques, climatiques et agricoles de l’évolution des prix alimentaires mondiaux entre 2015 et 2025.

L’objectif principal est d’identifier les facteurs les plus influents expliquant la fluctuation des prix alimentaires à l’échelle mondiale, à travers une approche basée sur l’analyse de données, la modélisation statistique et la visualisation interactive.


## Problématique

Quels sont les principaux facteurs économiques et climatiques qui influencent l’évolution des prix alimentaires mondiaux entre 2015 et 2025 ?

## Objectifs

- Identifier les variables les plus influentes
- Mesurer les corrélations entre climat, économie et prix alimentaires
- Comparer les effets selon les régions du monde
- Analyser l’impact de la production agricole sur la dynamique des prix
- Développer un dashboard interactif pour la visualisation des résultats


## Sources de Données

### Données Économiques
Source : World Bank Open Data  
https://data.worldbank.org

Indicateurs :
- CPI (Indice des prix à la consommation)
- PPI (Indice des prix à la production)
- PIB
- PIB par habitant
- Inflation
- Population

### Données Agricoles
Source : FAOSTAT  
https://www.fao.org/faostat

- Production Quantity (tonnes)
- Gross Production Value (constant 2014–2016 thousand US$)

### Données Climatiques
Source : World Bank Climate Portal  
https://climateknowledgeportal.worldbank.org

- Température moyenne annuelle
- Précipitations annuelles


## Outils Utilisés

- Python (pandas, numpy, matplotlib, seaborn)
- Power BI
- Git & GitHub
- Trello (gestion de projet)  
  [Voir le tableau Trello](https://trello.com/invite/b/695bbe78740b532feaa6d969/ATTI65efd190f31ba0e88e6a91620bfadea4610D7668/analyse-statistique-avancee-des-accidents-de-la-route-aux-etats-unis-assigne)

- Confluence (documentation)  
  [Voir la documentation Confluence](https://fatimazahrabellala.atlassian.net/wiki/x/AQBBAQ)

## Résultats Attendus

- Identification des facteurs dominants (économiques vs climatiques)
- Modèle de régression statistiquement significatif
- Tableau de bord interactif
- Rapport analytique détaillé

